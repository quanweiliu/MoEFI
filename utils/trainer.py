...
def train_morphFormer2(net, contra_head, super_head, awl, criterion1, criterion2, \
         contrastive_loader, train_loader, train_optimizer, args):
    net.train()
    contra_head.train()
    super_head.train()
    ...
    train_accuracy = 100. * train_correct.item() / len(train_loader.dataset)
    train_time = time.time() - start_time
    # print(loss_mutual2.item(), loss_mutual3.item())
    return round(loss.item(), 4), \
            round(loss_contra.item(), 4), \
            round(loss_orth.item(), 4), \
            round(loss_super.item(), 4), \
...
def trainer(net, contra_head, super_head, awl, criterion2, criterion4, \
    ...
    #     ########## contra ####################################>>>>>>>>>>>>>>>
        ...
        loss_contra2 = criterion4(u_f21, u_f22)
        loss_contra3 = criterion5(u_1f, u_2f, [u_f11, u_f12, u_f21, u_f22])
        # loss_contra = contra_loss1 + contra_loss2
        # loss_contra = args.lambda_contra*(loss_contra1 + loss_contra2)
