...
def linear_test_cnn(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    ...
    with torch.no_grad():
        ...
        # print('Accuracy: {}/{} ({:.2f}%)\n'.format(correct, len(data_loader.dataset), test_accuracy))
...
def linear_test_cnn2(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
...
def linear_test_mine_combine(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
    ...
    with torch.no_grad():
        ...
        # for data, _, target in data_loader:
            ...
            s_add1, s_add2, s_fuse1, s_fuse2 = net(s_data11, s_data21, s_data12, s_data22, s_data13, s_data23)
            s_out_1, s_out_2, s_out_3 = super_head(s_fuse1, s_fuse2)
            s_out = s_out_1 + s_out_2 + s_out_3
...
def linear_test_mine_gate(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
    ...
    with torch.no_grad():
        ...
        # for data, _, target in data_loader:
            ...
            s_add1, s_add2, gate_fused = net(s_data11, s_data21, s_data12, s_data22, s_data13, s_data23)
            s_out = super_head(gate_fused)
...
def linear_test_mine_combine_gate(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
    ...
    with torch.no_grad():
        ...
        # for data, _, target in data_loader:
            ...
            s_add1, s_add2, gate_fused, s_fuse1, s_fuse2 = net(s_data11, s_data21, s_data12, s_data22, s_data13, s_data23)
            logits, s_out1, s_out2 = super_head(gate_fused, s_fuse1, s_fuse2)
            s_out = logits + s_out1 + s_out2

            test_loss = criterion2(logits, target).item()
            test_pred = s_out.data.max(1, keepdim=True)[1]
            # test_pred = torch.argmax(s_out, dim=1)  # 这一行和上面的实现效果是一样的
...
def linear_test_mine(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
...
def linear_test_mine2(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
...
def linear_test_mine3(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
    ...
    with torch.no_grad():
        ...
        # for data, _, target in data_loader:
            ...
            s_add1, s_add2, gate_fused, s_fuse1, s_fuse2 = net(s_data11, s_data21, s_data12, s_data22, s_data13, s_data23)
            s_out1, s_out2, logits, s_out3, s_out4 = super_head(s_add1, s_add2, gate_fused, s_fuse1, s_fuse2)
            s_out = logits + s_out3 + s_out4
...
def linear_test_mine_cnn(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
    ...
    with torch.no_grad():
        ...
        # for data, _, target in data_loader:
            ...
            s_base1, s_base2, gate_fused3, s_fuse1, s_fuse2 = net(S_1, S_2)
            s_out1, s_out2, logits, s_out3, s_out4 = super_head(s_base1, s_base2, gate_fused3, s_fuse1, s_fuse2)
            s_out = logits + s_out3 + s_out4
...
def linear_test_mine_cnn_gate(net, super_head, criterion2, data_loader, args, groundTruth=None, visulation=False):
    net.eval()
    super_head.eval()
